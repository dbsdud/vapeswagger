...
class ImageReg extends Component{
    constructor(props) {
        super(props)
        this.state= { 
            editorHtml: '', 
    ...
    async noticeSubmit(){
        if(this.state.title === "") {
            ...
            try{
                ...
                const submit = await axios.post('http://15.164.160.236:8080/imageBoards/noticeSubmit', {
                    imageBoardTitle: this.state.title,
                    imageBoardContent: this.state.editorHtml,
                    imageBoardWriter: this.state.uDTO.userNickName,
                    userNo : this.state.uDTO.userNo 
                })
                if(submit.status === 200) {
                    alert("게시물이 등록되었습니다")
                    this.setState({
                        noticeListRender:  <Fragment><ImageBoardList/></Fragment>
                    })
