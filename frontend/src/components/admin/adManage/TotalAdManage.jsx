...
class TotalAdManage extends Component{

    // 광고 등록 : AdRegister
    // 전체 광고관리 : TotalAdManage
    // 노출 광고관리 : EnableAdManage
    // 비활성 광고관리 : DisableAdManage
    state= {
        adListResponse:[
            {
                adNo:"",   //광고번호
    ...
    async componentWillMount(){
        const response = await axios.get('http://15.164.160.236:8080/admanage/totalAdmanage/1')
        this.setState({
    ...
    async handleSearch() {
        ...
        const response = await axios.get(`http://15.164.160.236:8080/admanage/adSearch/total/${adSearch}/1`)
    ...
    async handleDisable() {
        ...
        adNoList.push(this.state.adListResponse[size]="disable")
        const response = await axios.put(`http://15.164.160.236:8080/admanage/adEnDisable`, adNoList)
        if(response.status ===200) {
            alert("비활성화 되었습니다.")
            this.props.history.push('/disableadmanage')
        } else {
            alert("실패하였습니다.")
        }
    }
    ...
    async pagingPrev() {
        ...
        if(adSearch===""){
            const response = await axios.get(`http://15.164.160.236:8080/admanage/totalAdmanage/${page}`)
            this.setState({
                adListResponse:response.data.adList,
                pagingResponse:response.data.paging,
                countResponse:response.data.paging.listCnt
            ...
            const response = await axios.get(`http://15.164.160.236:8080/admanage/adSearch/total/${adSearch}/${page}`)
            if(response.status ===200) {
                this.setState({
                    adListResponse:response.data.adSearchList,
                    pagingResponse:response.data.paging,
            ...
            return (
                <div className="adminCheckbox" key={adRow.adNo}>
                    <label className="adminCol-md-4 adminTbody">
                        <Link to={{pathname:`/adDetail/${adRow.adNo}`}}><img src={adRow.adImg} style={{width: '100%', height: '100px', margin: '10px 0px', objectFit: 'contain'}} alt="광고 이미지" title="상세보기"/></Link>
                    ...
                    <div className="adminContainer">
                        ...
                        <div className="adminCol-md-4 adminMenu">
                            <Link to="/TotalAdManage">
                                <img src="http://placehold.it/50x50" alt="테스트 이미지"/>
                                <h4 className="adminActive">광고관리</h4>
                            </Link>
                        </div>
                    </div>
                    <div className="adminLnb-sub">
                        <div className="adminContainer">
                            <Link to="/TotalAdManage" className="adminActive">전체 광고관리</Link>
                            <Link to="/EnableAdManage">노출 광고관리</Link>
                            <Link to="/DisableAdManage">비활성 광고관리</Link>
                        </div>
                    </div>
                </div>
