...
public class AdmanageController {
	...
	public ResponseEntity<HashMap<String,Object>> getExposeAd() throws Exception{
		...
		// 1. 광고가 있을 경우
		if(adExposeList.size() != 0){
			System.out.println("광고가 있다");
			int adRandomExposeNo = (int) (Math.random()*adExposeList.size()); //0<= x <5  1<= x <2 
			aDTO = adExposeList.get(adRandomExposeNo);
			System.out.println("          광고 번호 : "+aDTO.getAdNo());
			System.out.println("광고 잔여 노출 횟수 : "+aDTO.getAdEnable());
			System.out.println("   광고 활성화 여부 : "+aDTO.getAdActive());
			adNo = aDTO.getAdNo();
			// 1-1. 디폴트 광고일 경우
			if(adNo.equals("1")){
				adExposeList = admanageService.getDefault(adNo);
			} else { // 1-2. 디폴트 광고가 아닐 경우
				//메인 광고 노출 카운트 1 감소하는 서비스 
				int result  = admanageService.updateMainAdExposeCountUp(adNo);
				// 광고 단일 잔여 노출 횟수 조회
				int count = admanageService.getCount(adNo);
				//광고 노출 횟수가 0 이면 완료 컬럼
				if(count==0){
					admanageService.updateActive(adNo);
				}
			}
		} else { // 2. 광고가 없을 경우
			System.out.println("광고가 없다");
			// 디폴트 광고 가져오기
			adNo = "1";
			adExposeList = admanageService.getDefault(adNo);
		}
